...
import { Printer, CheckCircle, AlertTriangle, Barcode, Tag } from "lucide-react";
...
const LabelPrinting = () => {
  ...
  const [labelType, setLabelType] = useState<"rfid" | "barcode">("rfid");
  ...
  const handlePreview = () => {
    ...
    toast({
      ...
      description: `${quantity} ${labelType === "rfid" ? "RFID" : "barcode"} labels ready for printing`,
    });
  };
  ...
  const handlePrint = () => {
    ...
    toast({
      ...
      description: `Printing ${previewData.length} ${labelType === "rfid" ? "RFID" : "barcode"} labels...`,
    });
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description: `Successfully printed ${previewData.length} ${labelType === "rfid" ? "RFID" : "barcode"} labels`,
      });
    }, 2000);
  };
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      <div className="flex items-center justify-between">
        <h2 className="text-3xl font-bold tracking-tight">Label Printing</h2>
      </div>

      <Tabs value={labelType} onValueChange={(value) => setLabelType(value as "rfid" | "barcode")}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              <CardTitle>
                {labelType === "rfid" ? "Print RFID Labels" : "Print Barcode Labels"}
              </CardTitle>
              <CardDescription>
                {labelType === "rfid" 
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Preview of {labelType === "rfid" ? "RFID" : "barcode"} labels to be printed
              </CardDescription>
            </CardHeader>
            <CardContent>
              {previewData.length > 0 ? (
                    ...
                    {previewData.slice(0, 6).map((binId, index) => (
                      ...
                      >
                        <div className="text-xs text-gray-500 mb-1">
                          {labelType === "rfid" ? "RFID Bin Label" : "Barcode Label"}
                        </div>
                        <div className="text-lg font-bold">{binId}</div>
                        {labelType === "rfid" ? (
                          <div className="mt-2 bg-gray-800 text-white px-4 py-1 text-xs rounded-sm">
                            {binId}
                          </div>
                        ) : (
                          <div className="mt-2">
                ...
                <div className="h-[300px] flex flex-col items-center justify-center text-muted-foreground">
                  {labelType === "rfid" ? (
                    <Tag className="h-16 w-16 mb-4 text-muted" />
                  ) : (
